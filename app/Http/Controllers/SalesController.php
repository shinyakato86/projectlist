...
class SalesController extends Controller
{
    ...
    public function sales_personal(Request $request)
    {
      ...
      if ($request->filled('seach_year') && $request->filled('seach_month')) {
        ...
        $sales_personal = \DB::table('projectlists')->join('creators', 'projectlists.id', '=', 'creators.id')->selectRaw('creator_name , sum(creator_price) as amount, accounting_date')->whereYear('accounting_date', $year)->whereMonth('accounting_date', $month)->groupByRaw('creator_name')->orderBy('amount', 'desc')->get();
    ...
    public function sales_client(Request $request)
    {
      ...
      if ($request->filled('seach_year') && $request->filled('seach_month')) {
        ...
        $sales_client = \DB::table('projectlists')->selectRaw('client_name , sum(price) as amount, accounting_date')->whereYear('accounting_date', $year)->whereMonth('accounting_date', $month)->groupByRaw('client_name')->orderBy('amount', 'desc')->get();
    ...
    public function sales_category(Request $request)
    {
      ...
      if ($request->filled('seach_year') && $request->filled('seach_month')) {
        ...
        $sales_category = \DB::table('projectlists')->join('creators', 'projectlists.id', '=', 'creators.id')->selectRaw('creator_category , sum(creator_price) as amount, accounting_date')->whereYear('accounting_date', $year)->whereMonth('accounting_date', $month)->groupByRaw('creator_category')->orderBy('amount', 'desc')->get();
    ...
    public function sales_department(Request $request)
    {
      ...
      if ($request->filled('seach_year') && $request->filled('seach_month')) {
        ...
        $sales_department = \DB::table('projectlists')->selectRaw('department_name , sum(price) as amount, accounting_date')->whereYear('accounting_date', $year)->whereMonth('accounting_date', $month)->groupByRaw('department_name')->orderBy('amount', 'desc')->get();
